...
Widget richTextLink({required String title, required String linkText, required GestureTapCallback onTap}) {
  return RichText(
    text: TextSpan(
      text: title,
      style: TextStyle(
...
Widget itemContain({required String label, required bool isOk}) {
  return Row(
    children: [
      isOk ? SvgPicture.asset(AssetIcons.checkGreen) : SvgPicture.asset(AssetIcons.checkGrey),
      SizedBox(width: 8),
      Text(
        label,
        style: TextStyle(color: isOk ? AppColors.mainText : AppColors.secondaryText),
      ),
    ],
  );
}
...
Future dialog(
    {required String title, required String subtitle, required Widget icon, required String txtButton, required GestureTapCallback onPressed}) {
